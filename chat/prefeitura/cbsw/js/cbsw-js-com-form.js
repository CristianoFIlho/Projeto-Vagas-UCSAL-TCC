...
function PrimeiraResposta() {
  ...
  if(aberto != true){
    ...
    var url = "https://assistente.freire.com.br/sofia/respondemobileportal.rule?proj=970e5c92-674e-4e55-9d0d-6f8411fc8177&sys=SSA&pergunta=&latitude=&longitude=&id=undefined&ini=s&usu=APA91bEMuk931CWQUeu340pyKgQ83jR1lcZLMWlYZZNIgkQcI-n65Roxauy_ukoOJe4sNGm9C3HvtB6RWr-es6UZ0n0JnHf6Dr3FjHmqdOJQREzwJ_kYHt8&sessao="+sessao
    
    var data = new Date();
    
    if((data.getHours() > 0) && (data.getHours() < 13)){
    //BOM DIA
    document.getElementById('cbsw_dialogue').innerHTML = "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-info d-inline-block shadow'><strong class='d-block'>Eduardo</strong>Bom dia, eu sou o Eduardo! O que você deseja saber?</span></div>";   
        }else if((data.getHours() > 13) && (data.getHours() < 18)){
    //BOA TARDE
    document.getElementById('cbsw_dialogue').innerHTML = "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-info d-inline-block shadow'><strong class='d-block'>Eduardo</strong>Boa tarde, eu sou o Eduardo! O que você deseja saber?</span></div>";     
        }else{
    //BOA NOITE
    document.getElementById('cbsw_dialogue').innerHTML = "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-info d-inline-block shadow'><strong class='d-block'>Eduardo</strong>Boa noite, eu sou o Eduardo! O que você deseja saber?</span></div>"; 
        }
...
function resposta(texto,id) {    
    var historico = document.getElementById('cbsw_dialogue').innerHTML;    
    var url = "https://assistente.freire.com.br/sofia/respondemobileportal.rule?proj=970e5c92-674e-4e55-9d0d-6f8411fc8177&sys=SSA&pergunta="+URLEncode2(texto)+"&latitude=&longitude=&id=undefined&ini=n&usu=APA91bEMuk931CWQUeu340pyKgQ83jR1lcZLMWlYZZNIgkQcI-n65Roxauy_ukoOJe4sNGm9C3HvtB6RWr-es6UZ0n0JnHf6Dr3FjHmqdOJQREzwJ_kYHt8&sessao="+sessao
	...
	var retorno = $.get(url, function(data, status) {  
    	var indice = data.indexOf('||||');
    	var res = data.substr(0,indice);
    	document.getElementById('cbsw_dialogue').innerHTML = historico + "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-info d-inline-block shadow'><strong class='d-block'>Eduardo</strong>"+res+"</span></div>";
    })
...
function Acessar(login,senha){
    ...
    var url = "https://assistente.freire.com.br/sofia/SofiaLogarFalaSSA.rule?sys=SSA&workspace=970e5c92-674e-4e55-9d0d-6f8411fc8177&login="+login+"&senha="+senha+"&sessao="+sessao
    ...
    .fail(function() {
        document.getElementById('cbsw_dialogue').innerHTML = historico + " <div class='cbsw_rr cbsw_request my-2'><span class='cbsw_bubble alert alert-warning d-inline-block'><strong class='d-block'>Você</strong>"+'Não foi possível enviar a sua dúvida para o Eduardo. Aqui está uma lista de possíveis motivos: bloqueio de palavras em seu firewall ou proxy ou sem conexão entre você e o servidor.'+"</span></div>";     
        document.getElementById('cbsw_dialogue').scrollTop = 100000000;
    })
}
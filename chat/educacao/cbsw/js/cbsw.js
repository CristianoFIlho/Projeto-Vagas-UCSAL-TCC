...
function PrimeiraResposta() {
  ...
  if(aberto != true){
    ...
    var url = "http://sofia.softwell.com.br/sofia/respondemobileportal.rule?proj=068dfdf7-b610-4a61-895e-cc336e9f44bf&sys=SSA&pergunta=&latitude=&longitude=&id=undefined&ini=s&usu=APA91bEMuk931CWQUeu340pyKgQ83jR1lcZLMWlYZZNIgkQcI-n65Roxauy_ukoOJe4sNGm9C3HvtB6RWr-es6UZ0n0JnHf6Dr3FjHmqdOJQREzwJ_kYHt8&sessao="+sessao
    
    var data = new Date();
    
    if((data.getHours() > 0) && (data.getHours() < 13)){
    //BOM DIA
    document.getElementById('cbsw_dialogue').innerHTML = "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-info d-inline-block shadow'><strong class='d-block'>Selena</strong>Bom dia, eu sou a Selena! O que você deseja saber?</span></div>";   
        }else if((data.getHours() > 13) && (data.getHours() < 18)){
    //BOA TARDE
    document.getElementById('cbsw_dialogue').innerHTML = "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-info d-inline-block shadow'><strong class='d-block'>Selena</strong>Boa tarde, eu sou a Selena! O que você deseja saber?</span></div>";     
        }else{
    //BOA NOITE
    document.getElementById('cbsw_dialogue').innerHTML = "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-info d-inline-block shadow'><strong class='d-block'>Selena</strong>Boa noite, eu sou a Selena! O que você deseja saber?</span></div>"; 
        }
...
function resposta(texto,id) {    
    var historico = document.getElementById('cbsw_dialogue').innerHTML;    
    var url = "http://sofia.softwell.com.br/sofia/respondemobileportal.rule?proj=068dfdf7-b610-4a61-895e-cc336e9f44bf&sys=SSA&pergunta="+URLEncode2(texto)+"&latitude=&longitude=&id=undefined&ini=n&usu=APA91bEMuk931CWQUeu340pyKgQ83jR1lcZLMWlYZZNIgkQcI-n65Roxauy_ukoOJe4sNGm9C3HvtB6RWr-es6UZ0n0JnHf6Dr3FjHmqdOJQREzwJ_kYHt8&sessao="+sessao
	...
	var retorno = $.get(url, function(data, status) {  
    	var indice = data.indexOf('||||');
    	var res = data.substr(0,indice);
    	document.getElementById('cbsw_dialogue').innerHTML = historico + "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-info d-inline-block shadow'><strong class='d-block'>Selena</strong>"+res+"</span></div>";
    })
...
function Acessar(login,senha){
    ...
    var url = "http://sofia.softwell.com.br/sofia/SofiaLogarFalaSSA.rule?sys=SSA&workspace=e865764a-db3b-4d1f-b55e-4b9c624d2237&login="+login+"&senha="+senha+"&sessao="+sessao

...
function PrimeiraResposta() {
  ...
  if(aberto != true){
    ...
    //BOM DIA
    document.getElementById('cbsw_dialogue').innerHTML = "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-danger d-inline-block shadow'><strong class='d-block'>Regina</strong>Bom dia, eu sou a Regina! O que você deseja saber sobre o Maker?</span></div>";   
        }else if((data.getHours() > 13) && (data.getHours() < 18)){
    //BOA TARDE
    document.getElementById('cbsw_dialogue').innerHTML = "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-danger d-inline-block shadow'><strong class='d-block'>Regina</strong>Boa tarde, eu sou a Regina! O que você deseja saber sobre o Maker?</span></div>";     
        }else{
    //BOA NOITE
    document.getElementById('cbsw_dialogue').innerHTML = "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-danger d-inline-block shadow'><strong class='d-block'>Regina</strong>Boa noite, eu sou a Regina! O que você deseja saber sobre o Maker?</span></div>"; 
        }
...
function resposta(texto,id) {    
	...
	var retorno = $.get(url, function(data, status) {  
    	var indice = data.indexOf('||||');
    	var res = data.substr(0,indice);
    	document.getElementById('cbsw_dialogue').innerHTML = historico + "<div class='cbsw_rr cbsw_return my-2'><span class='cbsw_bubble alert alert-danger d-inline-block shadow'><strong class='d-block'>Regina</strong>"+res+"</span></div>";
    })
...
function Acessar(login,senha){
    ...
    .fail(function() {
        document.getElementById('cbsw_dialogue').innerHTML = historico + " <div class='cbsw_rr cbsw_request my-2'><span class='cbsw_bubble alert alert-warning d-inline-block'><strong class='d-block'>Você</strong>"+'Não foi possível enviar a sua dúvida para a Regina. Aqui está uma lista de possíveis motivos: bloqueio de palavras em seu firewall ou proxy ou sem conexão entre você e o servidor.'+"</span></div>";     
        document.getElementById('cbsw_dialogue').scrollTop = 100000000;
    })
}